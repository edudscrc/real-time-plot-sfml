...
void Plot::createSubplots(size_t numRows, size_t numCols, float yMinValue, float yMaxValue, float offsetX,
                          float offsetY)
{
    ...
    this->m_subplotSizeX =
        (static_cast<float>(this->m_windowWidth) - ((numCols + 1) * offsetX)) / static_cast<float>(numCols);
    this->m_subplotSizeY =
        (static_cast<float>(this->m_windowHeight) - ((numRows + 1) * offsetY)) / static_cast<float>(numRows);
...
void Plot::show()
{
    ...
    for (auto &grid : this->m_axes)
    {
        ...
        for (auto &point : grid.m_points)
        {
            this->m_window.draw(point);
            if (point.getPosition().x > grid.min.x)
            {
                point.move({-1.f, 0.f});
            }
        }
...
void Plot::sendValueToGrid(size_t gridIdx, const double value)
{
    this->m_axes[gridIdx].send(value);
}